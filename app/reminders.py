...
class ReminderService:
    ...
    def schedule_for_doctor(self, telegram_user_id: int, doctor_name: str, due_at: datetime, label: str) -> ReminderRecord:
        self.cancel_for_user(telegram_user_id)
    ...
    def cancel_for_user(self, telegram_user_id: int) -> None:
        job = self._jobs.pop(telegram_user_id, None)
        if job is not None:
            job.cancel()

    def _schedule(self, reminder: ReminderRecord) -> None:
        self.cancel_for_user(reminder.telegram_user_id)
        self._jobs[reminder.telegram_user_id] = asyncio.create_task(self._run(reminder))
    ...
    async def _run(self, reminder: ReminderRecord) -> None:
        try:
            ...
            self._jobs.pop(reminder.telegram_user_id, None)
    ...
    async def _send(self, reminder: ReminderRecord) -> None:
        ...
        if tasks:
            ...
            reply_markup = reminder_only_keyboard()

        await self._bot.send_message(reminder.telegram_user_id, text, reply_markup=reply_markup)
